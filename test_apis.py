...
def test_database():
    """
    Prueba la funcionalidad de la base de datos local
    """
    print("Probando base de datos local...")
    ...
    try:
        ...
        expected_tables = {'labor_data', 'series_metadata'}
        actual_tables = {table[0] for table in tables}
        
        if not expected_tables.issubset(actual_tables):
            print(f"ERROR: Faltan tablas en la base de datos")
        ...
        print(f"OK: Base de datos funcionando correctamente")
...
def main():
    ...
    if args.all or not any([args.fred, args.bls, args.db]):
        total_tests += 1
        if test_all_metrics():
            success_count += 1
        print()
